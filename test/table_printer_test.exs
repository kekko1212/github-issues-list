defmodule TablePrinterTest do
  ...
  alias Issues.TablePrinter, as: TP
  ...
  def split_with_tree_columns do
    TP.split_into_columns(simple_test_data(), headers())
  end

  test "Split into 3 columns" do
  ...
  test "column_widths" do
    widths = TP.widths_of(split_with_tree_columns())
    assert widths == [5,6,7]
  end

  test "correct format string returned" do
    assert TP.format_for([9,10,11]) == "~-9s | ~-10s | ~-11s~n"
  end

  test "Output is correct" do
    result = capture_io fn -> TP.print_table_for_columns(simple_test_data(), headers()) end
    assert result == """
    c1    | c2     | c4     
    ------+--------+--------
    r1 c1 | r1 c2  | r1+++c4
